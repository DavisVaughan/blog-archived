...
[Don't forget to remove the `-O0` flag, or revert it back to `-O2` after you are finished debugging! Forgetting to revert this flag can sometimes result in performance degradation for packages that you compile from source, because you are telling the compiler not to optimize the code.]{.marginnote}
...
```{r, echo = FALSE}
blogdown::shortcode("youtube", id = "6zY0qrcKTL0", autoplay = "true")
```
