...
While this works, it lacks the elegance and ease of use that we typically expect from a full fledged debugger, i.e. setting breakpoints by clicking on the line we care about, clicking to step over or into functions, etc.
<!-- jenny comment -->
...
<!-- is there any expository reason for `size` to be `cpp11::integers` instead of plain old `int`?
...
```{r, echo=FALSE}
blogdown::shortcode("youtube", id = "_u5k-Fk2bfU", autoplay = "true")
```
...
At a minimum, for this C++11 based project we need to add `CXX11FLAGS=-O0 -g` to this file to instruct our compiler to compile without optimization (that's `-O0`) and include all debugging information (the `-g`).
<!-- jenny comment -->
...
[Don't forget to either remove these flags, or revert back to `-O2` after you are finished debugging!]{.marginnote}

<!-- jenny comment -->

```{r, echo=FALSE}
blogdown::shortcode("youtube", id = "ihkLGPSkVMY", autoplay = "true")
```
...
-   `request`: Whether this is a `launch` or `attach` type of configuration. We want VS Code to *launch* a new R session for us, but we can also have it *attach* to an existing one using its process ID (PID). We will keep this as launch for now, but at the end of this post I'll also show an attach example.
...
-   `args`: Arguments to supply to the `program`. Here we supply `--vanilla` to use a vanilla R session which doesn't load in any history or saved workspaces and also does not load the usual startup files. We then tell it to execute the command `source('tools/debug.R')` when it starts up the R session.
...
<!-- say something about the choice of `tools/` for this? also important to Rbuildignore `tools/` -->
...
It doesn't use the typical lldb inspection tools, but instead uses the fact that I can step one line at a time to figure out where the exact issue is occuring from.
...
```{r, echo=FALSE}
blogdown::shortcode("youtube", id = "43QxsQRluBQ", autoplay = "true")
```
...
<!-- I would still include the "explicit instantiation" move here. It feels safe to say that it's important to know about and that we've had partial success with this, although not universal. Plus, someone's "well, actually" this if you don't mention it. -->
...
-   A `.vscode` folder with `launch.json` and and `c_cpp_properties.json`
